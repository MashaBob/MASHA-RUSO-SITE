...
const talkNika=(text,voiceText=text)=>{if(guideMessage)guideMessage.textContent=text;if("speechSynthesis" in window){window.speechSynthesis.cancel();const utterance=new SpeechSynthesisUtterance(voiceText);utterance.lang="ru-RU";const voice=window.speechSynthesis.getVoices().find(item=>item.lang.toLowerCase().startsWith("ru"));if(voice)utterance.voice=voice;utterance.rate=.92;window.speechSynthesis.speak(utterance)}};
