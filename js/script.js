...
const talkNika=(text)=>{if(guideMessage)guideMessage.textContent=text;if("speechSynthesis" in window){window.speechSynthesis.cancel();const utterance=new SpeechSynthesisUtterance(text);utterance.lang="es-MX";utterance.rate=.96;window.speechSynthesis.speak(utterance)}};
const answerNika=(said)=>{const text=said.toLowerCase();let answer="Puedo ayudarte con tu nivel, con una clase, con la plataforma o para hablar con Masha.";if(/nivel|test|prueba/.test(text))answer="Perfecto. Te llevaré al test de nivel. Son diez preguntas y al final tendrás una recomendación hasta B2.";else if(/clase|horario|reserv|cita/.test(text))answer="Claro. Abriré la página de reserva para que elijas el día y la hora que prefieres.";else if(/juego|material|practic|plataforma/.test(text))answer="Vamos a la plataforma. Allí tienes una mini juego para practicar vocabulario ruso.";else if(/whatsapp|masha|pregunta|ayuda/.test(text))answer="Te conecto con Masha por WhatsApp para recibir ayuda personal.";talkNika(answer);if(/nivel|test|prueba/.test(text))setTimeout(()=>location.href="test-nivel.html",1200);else if(/clase|horario|reserv|cita/.test(text))setTimeout(()=>location.href="reservar.html",1200);else if(/juego|material|practic|plataforma/.test(text))setTimeout(()=>location.href="plataforma.html",1200);else if(/whatsapp|masha|pregunta|ayuda/.test(text))setTimeout(()=>window.open("https://wa.me/524421562187?text=Hola%20Masha%2C%20tengo%20una%20pregunta%20sobre%20los%20cursos.","_blank","noopener"),1200)};
if(guideSpeak)guideSpeak.addEventListener("click",()=>talkNika("Hola, soy Nika. Puedo ayudarte a descubrir tu nivel, reservar una clase, practicar con un juego o hablar con Masha."));
if(guideMic)guideMic.addEventListener("click",()=>{const Recognition=window.SpeechRecognition||window.webkitSpeechRecognition;if(!Recognition){talkNika("Tu navegador todavía no permite usar el micrófono aquí. Puedes usar los botones de arriba para elegir lo que necesitas.");return}const recognition=new Recognition();recognition.lang="es-MX";recognition.interimResults=false;recognition.maxAlternatives=1;talkNika("Te escucho.");recognition.onresult=e=>answerNika(e.results[0][0].transcript);recognition.onerror=()=>talkNika("No pude escucharte. Prueba otra vez o usa uno de los botones.");recognition.start()});