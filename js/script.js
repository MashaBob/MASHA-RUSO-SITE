...
const welcomeNika=()=>talkNika("Hola, me llamo Nika y seré tu ayudante durante todo el camino. Ya hiciste el primer paso al llegar aquí. Puedo mostrarte el test de nivel, ayudarte a reservar una clase, abrir la plataforma para practicar o hablar con la profesora.","Привет, меня зовут Ника, и я буду твоей помощницей на всём пути. Ты уже сделала первый шаг, раз пришла сюда. Я могу показать тест уровня, помочь записаться на урок, открыть платформу для практики или написать Маше.");
if(guideToggle&&guidePanel&&guideClose){const setGuide=open=>{guidePanel.classList.toggle("show",open);guideToggle.setAttribute("aria-expanded",String(open));if(open)setTimeout(welcomeNika,220)};guideToggle.addEventListener("click",()=>setGuide(!guidePanel.classList.contains("show")));guideClose.addEventListener("click",()=>setGuide(false))}
...
const talkNika=(text,voiceText=text)=>{if(guideMessage)guideMessage.textContent=text;if(!("speechSynthesis" in window))return;window.speechSynthesis.cancel();let spoken=false;const playBilingual=()=>{if(spoken)return;const voices=window.speechSynthesis.getVoices();const female=/female|femen|mujer|zira|paulina|dalia|helena|laura|sabina|monica|monica|irina|elena|katya|milena/i;const pick=prefix=>voices.find(v=>v.lang&&v.lang.toLowerCase().startsWith(prefix)&&female.test(v.name))||voices.find(v=>v.lang&&v.lang.toLowerCase().startsWith(prefix));const spanish=pick("es"),russian=pick("ru");if(!spanish&&!russian)return;spoken=true;const spanishText=text.replace(/\bNika\b/gi,"tu asistente").replace(/\bMasha\b/gi,"la profesora");for(const [value,voice] of [[spanishText,spanish],[voiceText,russian]]){if(!voice)continue;const u=new SpeechSynthesisUtterance(value);u.lang=voice.lang;u.voice=voice;u.rate=.92;window.speechSynthesis.speak(u)}};playBilingual();if(!spoken){if(guideMessage)guideMessage.textContent="Nika hablará en español y ruso cuando haya voces disponibles en este dispositivo.";window.speechSynthesis.addEventListener("voiceschanged",playBilingual,{once:true});setTimeout(playBilingual,900)}};
